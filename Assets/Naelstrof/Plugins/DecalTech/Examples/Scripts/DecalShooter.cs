...
public class DecalShooter : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, 10f, hitMask, QueryTriggerInteraction.Ignore)) {
            ...
            if (r != null) {
                PaintDecal.instance.RenderDecal(r, decal, hit.point + hit.normal * 0.25f, Quaternion.FromToRotation(Vector3.forward,-hit.normal), color, size, 0.5f, r.lightmapIndex == -1?true:false);
            }
        }
    }
}
